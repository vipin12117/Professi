...
class EDD_Points_Admin {
	...
	public function edd_points_meta_fields_save( $post_id ) {
		...
		update_post_meta( $post_id, '_edd_points_earned', $_POST['_edd_points_earned'] );
		
		//update maximum discount points
		update_post_meta( $post_id, '_edd_points_max_discount', $_POST['_edd_points_max_discount'] );
	}
	...
	public function edd_points_settings_validate( $input ) {
		...
		$input['edd_points_cart_max_discount'] 		= $this->model->edd_points_escape_slashes_deep( $input['edd_points_cart_max_discount'] );
		$input['edd_points_max_discount'] 			= $this->model->edd_points_escape_slashes_deep( $input['edd_points_max_discount'] );
		...
		$input['edd_points_earned_account_signup'] 	= $this->model->edd_points_escape_slashes_deep( $input['edd_points_earned_account_signup'] );
		...
		if ( isset( $_POST['edd_points_rewards'] ) ) {
			...
			foreach ( $cat_keys as $key ) {
				if ( isset ( $_POST['edd_points_rewards'][$key] ) ) {
					$eddpointscat[$key] = $this->model->edd_points_escape_slashes_deep( $_POST['edd_points_rewards'][$key] );
				}
			}

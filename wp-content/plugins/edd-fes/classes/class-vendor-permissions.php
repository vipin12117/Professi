...
class FES_Vendor_Permissions {
	...
	function vendor_can_edit_product( $post_id ) {
		if ( !EDD_FES()->fes_options->get_option( 'edd_fes_vendor_permissions_edit_product' ) ){
			return false;
		}
		$user_id = get_current_user_id();
		// TODO: setting for this
		$post = get_post( $post_id, ARRAY_A);
		if ( EDD_FES()->vendor_permissions->vendor_is_vendor( $user_id )  && (EDD_FES()->vendor_permissions->vendor_is_admin( $user_id ) || $post['post_author'] == $user_id) ){
			return true;
		}
		else {
			return false;
	...
	function vendor_can_delete_product( $post_id ) {
		if ( !EDD_FES()->fes_options->get_option( 'edd_fes_vendor_permissions_delete_product' ) ){
			return false;
		}
		$user_id = get_current_user_id();
		// TODO: setting for this
	...
	function vendor_is_admin( $user_id = -2 ) {
		if ( $user_id == -2 ) {
			$user_id = get_current_user_id();
		}
		if ( $user_id == 0 ) {
